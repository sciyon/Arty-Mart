...
const resolvers = {
  ...
  Mutation: {
    async userRegister(_, { registerUserInput: { email, password, fname, lname, gender, birthDate,  role, status, createdOn } }){
      ...
      const newUser =  new User({ email, password, token, fname, lname, gender, birthDate, role, status,  createdOn });
    ...
    async userUpdate(_, { ID, updateUserInput: { email, password, fname, lname, gender, birthDate,  status, role }}){
      try {
